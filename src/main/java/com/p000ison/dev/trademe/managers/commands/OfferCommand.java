            ...
            if (plugin.getTradeHandler().isTrading(player)) {
                if (args.length == 2) {
                    if (Util.checkItem(args[0])) {
                        plugin.getTradeHandler().requestOffer(player, Util.getPartner(plugin.getTradeHandler().getTrade(), player), new ItemStack(Material.valueOf(args[0].toUpperCase()), 1), Double.parseDouble(args[1]));
                    } else {
                        player.sendMessage(Util.color(String.format(plugin.getSettingsManager().getErrorInvalidItem(), args[0])));
                    }
                } else if (args.length == 3) {
                    if (Util.checkItem(args[0])) {
                        plugin.getTradeHandler().requestOffer(player, Util.getPartner(plugin.getTradeHandler().getTrade(), player), new ItemStack(Material.valueOf(args[0].toUpperCase()), Integer.parseInt(args[1])), Double.parseDouble(args[2]));
                    } else {
                        player.sendMessage(Util.color(String.format(plugin.getSettingsManager().getErrorInvalidItem(), args[0])));
                    }
                }
